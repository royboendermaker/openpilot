...
def create_pq_steering_control(packer, bus, apply_steer, CS, idx, lkas_enabled):
  values = {
    "HCA_Zaehler": idx,
    "LM_Offset": abs(apply_steer),
    "LM_OffSign": 1 if apply_steer < 0 else 0,
    "HCA_Status": 5 if CS.hcaSwitch else 3 if not (lkas_enabled and apply_steer !=0) else 7,
    "Vib_Freq": 16,
  }
